...
static inline int zmij_is_big_endian(void) {
...
static inline uint64_t zmij_uint128_to_u64(zmij_uint128 x) {
  return x.lo;
}

static inline zmij_uint128 zmij_uint128_rshift(zmij_uint128 x, int shift) {
  assert(shift >= 64 && shift < 128);
  zmij_uint128 result = {0, x.hi >> (shift - 64)};
  return result;
}

static inline zmij_uint128 zmij_uint128_add(zmij_uint128 lhs, zmij_uint128 rhs) {
  uint64_t lo = lhs.lo + rhs.lo;
  zmij_uint128 result = {lhs.hi + rhs.hi + (lo < lhs.lo), lo};
  return result;
}

#ifdef __SIZEOF_INT128__
typedef unsigned __int128 zmij_uint128_t;
#else
typedef zmij_uint128 zmij_uint128_t;
#endif

// 126-bit significands of strict overestimates of powers of 10.
// Generated with gen-pow10.py.
const zmij_uint128 zmij_pow10_significands[] = {
...
static inline zmij_uint128_t zmij_umul128(uint64_t x, uint64_t y) {
#ifdef __SIZEOF_INT128__
  return (zmij_uint128_t)x * y;
#else
  uint64_t a = x >> 32;
  ...
  zmij_uint128 result = {ac + (ad >> 32) + (bc >> 32) + (cs >> 32), (cs << 32) + (uint32_t)bd};
  return result;
#endif
}

// Computes upper 64 bits of multiplication of x and y, discards the least
// significant bit and rounds to odd, where x = zmij_uint128(x_hi << 64) | x_lo.
uint64_t zmij_umul192_upper64_inexact_to_odd(uint64_t x_hi, uint64_t x_lo, uint64_t y) {
#ifdef __SIZEOF_INT128__
  zmij_uint128_t result = zmij_umul128(x_hi, y) + (zmij_umul128(x_lo, y) >> 64);
  return (uint64_t)(result >> 64) | (((uint64_t)result >> 1) != 0);
#else
  zmij_uint128_t result = zmij_uint128_add(zmij_umul128(x_hi, y), zmij_uint128_rshift(zmij_umul128(x_lo, y), 64));
  return zmij_uint128_to_u64(zmij_uint128_rshift(result, 64)) | ((zmij_uint128_to_u64(result) >> 1) != 0);
#endif
}

// Converts value in the range [0, 100) to a string. GCC generates a bit better
// code when value is pointer-size (https://www.godbolt.org/z/5fEPMT1cc).
static inline const char* zmij_digits2(size_t value) {
...
static inline uint64_t zmij_digits2_u64(uint32_t value) {
  uint16_t digits;
  memcpy(&digits, zmij_digits2(value), 2);
  return digits;
}

// Converts the value `aa * 10**6 + bb * 10**4 + cc * 10**2 + dd` to a string
// returned as a 64-bit integer.
static uint64_t zmij_digits8_u64(uint32_t aa, uint32_t bb, uint32_t cc, uint32_t dd) {
  return zmij_digits2_u64(dd) << 48 | zmij_digits2_u64(cc) << 32 |
         zmij_digits2_u64(bb) << 16 | zmij_digits2_u64(aa);
}
...
static inline zmij_divmod_result zmij_divmod100(uint32_t value) {
  assert(value < 10000);
  static const int exp = 19;  // 19 is faster or equal to 12 even for 3 digits.
  static const int sig = (1 << exp) / 100 + 1;
  uint32_t div = (value * sig) >> exp;  // value / 100
  zmij_divmod_result result = {div, value - div * 100};
  return result;
}
...
size_t zmij_count_trailing_nonzeros(uint64_t x) {
  ...
  assert(!zmij_is_big_endian());
  // We count the number of characters until there are only '0' == 0x30
  // characters left.
  // The code is equivalent to
  //   return 8 - zmij_count_lzero(x & ~0x30303030'30303030) / 8
  ...
  //   return size_t(71 - zmij_count_lzero(x & ~0x30303030'30303030)) / 8;
  ...
  static const uint64_t mask_with_sentinel = (0x3030303030303030ull << 1) | 1;
  return (size_t)(70 - zmij_count_lzero((x << 1) ^ mask_with_sentinel)) / 8;
}

// Writes a significand consisting of 16 or 17 decimal digits and removes
// trailing zeros.
static char* zmij_write_significand(char* buffer, uint64_t value) {
  ...
  zmij_divmod_result abb_res = zmij_divmod100(abb);
  uint32_t a = abb_res.div;
  uint32_t bb = abb_res.mod;
  zmij_divmod_result ddee_res = zmij_divmod100(ddee);
  uint32_t dd = ddee_res.div;
  uint32_t ee = ddee_res.mod;
  ...
  *buffer = (char)('0' + a);
  ...
  // one go and zmij_count_trailing_nonzeros doesn't have to load from memory.
  uint64_t digits = zmij_digits8_u64(bb, cc, dd, ee);
  memcpy(buffer, &digits, 8);
  if (ffgghhii == 0) return buffer + zmij_count_trailing_nonzeros(digits);

  buffer += 8;
  uint32_t ffgg = ffgghhii / 10000;
  uint32_t hhii = ffgghhii % 10000;
  zmij_divmod_result ffgg_res = zmij_divmod100(ffgg);
  uint32_t ff = ffgg_res.div;
  uint32_t gg = ffgg_res.mod;
  zmij_divmod_result hhii_res = zmij_divmod100(hhii);
  uint32_t hh = hhii_res.div;
  uint32_t ii = hhii_res.mod;
  digits = zmij_digits8_u64(ff, gg, hh, ii);
  memcpy(buffer, &digits, 8);
  return buffer + zmij_count_trailing_nonzeros(digits);
}

// Writes the decimal FP number dec_sig * 10**dec_exp to buffer.
static void zmij_write(char* buffer, uint64_t dec_sig, int dec_exp) {
  dec_exp += 15 + (dec_sig >= (uint64_t)1e16);

  char* start = buffer;
  buffer = zmij_write_significand(buffer + 1, dec_sig);
  start[0] = start[1];
  start[1] = '.';
  ...
  *buffer++ = sign;
  zmij_divmod_result exp_res = zmij_divmod100((uint32_t)dec_exp);
  uint32_t a = exp_res.div;
  uint32_t bb = exp_res.mod;
  *buffer = (char)('0' + a);
  buffer += dec_exp >= 100;
  memcpy(buffer, zmij_digits2(bb), 2);
  buffer[2] = '\0';
}
...
void zmij_dtoa(double value, char* buffer) {
  ...
  *buffer = '-';
  ...
  static const int log10_3_over_4_sig = -131008;
  // log10_2_sig = round(log10(2) * 2**log10_2_exp)
  static const int log10_2_sig = 315653;
  static const int log10_2_exp = 20;
  ...
  static const int dec_exp_min = -292;
  zmij_uint128 pow10 = zmij_pow10_significands[-dec_exp - dec_exp_min];
  uint64_t pow10_hi = pow10.hi;
  uint64_t pow10_lo = pow10.lo;

  // log2_pow10_sig = round(log2(10) * 2**log2_pow10_exp) + 1
  static const int log2_pow10_sig = 217707;
  static const int log2_pow10_exp = 16;
  ...
  // Shift to ensure the intermediate result in zmij_umul192_upper64_inexact_to_odd
  ...
  lower = zmij_umul192_upper64_inexact_to_odd(pow10_hi, pow10_lo, lower << shift) +
          bin_sig_lsb;
  upper = zmij_umul192_upper64_inexact_to_odd(pow10_hi, pow10_lo, upper << shift) -
          bin_sig_lsb;
  ...
  if ((shorter << 2) >= lower) {
    zmij_write(buffer, shorter, dec_exp);
    return;
  }

  uint64_t scaled_sig = zmij_umul192_upper64_inexact_to_odd(
  ...
  zmij_write(buffer, (under_closer & under_in) ? dec_sig_under : dec_sig_over,
        dec_exp);
}
